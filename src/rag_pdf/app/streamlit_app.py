...
from rag_pdf.app.image_tab import render_image_tab
        ...
        with header[3]:
            with st.popover("➕", help="Criar subpasta aqui"):
                ...
                if st.button("Criar", key=f"newsub_btn::{full_path}") and new_sub.strip():
                    ...
                    st.rerun()
        with header[4]:
            delete_folder_key = f"delete_folder::{full_path}"
            if confirm_icon_action(
                "🗑️", key=delete_folder_key, help="Apagar pasta e conteúdo"
            ):
                ...
                st.rerun()
        with header[5]:
            open_folder = st.toggle(
                "Abrir", key=f"open_folder::{full_path}", label_visibility="collapsed"
            )
...
tab_pdf, tab_images = st.tabs(["📄 PDFs", "🖼️ Imagens"])
    ...
    with st.sidebar:
        ...
        with st.expander("📁 Escolher pasta de destino", expanded=False):
            ...
            new_folder_name = st.text_input(
                "Criar nova pasta (ex.: Suporte/2024)", key="new_folder_name_input"
            )
        ...
        mode = st.radio(
            "Como responder às perguntas do chat", [TOPK_MODE, ADAPTIVE_MODE, MAPREDUCE_MODE]
        )
        ...
        if mode == TOPK_MODE:
            st.caption(
                "Busca sempre os k trechos mais parecidos com a pergunta — rápido, "
                "bom para fatos pontuais."
            )
            top_k = st.number_input(
                "Quantos trechos buscar (k)", min_value=1, max_value=50, value=settings.top_k
            )
            ...
            profile_labels = {
                **{k: p.label for k, p in RETRIEVAL_PROFILES.items()},
            ...
            if profile_key == "personalizado":
                margin_pct = st.slider(
                    "Margem de similaridade (%)", min_value=0, max_value=100, value=5, step=1
                )
                max_context_chars = st.slider(
                    "Teto de contexto (caracteres)",
                    min_value=500,
                    max_value=20_000,
                    value=6000,
                    step=500,
                ...
                profile = RetrievalProfile(
                    "Personalizado", margin_pct=margin_pct, max_context_chars=max_context_chars
                )
            else:
            ...
            if documents:
                summarize_source = st.selectbox(
                    ...
                    format_func=lambda s: next(
                        (
                            f"{d['folder']}/{s}" if d["folder"] else s
                            for d in documents
                            if d["source"] == s
                        ),
                        s,
        ...
        if mode in (TOPK_MODE, ADAPTIVE_MODE):
            ...
            if selected_sources:
                st.caption(
                    f"🗂️ Escopo: {len(selected_sources)} de {total_docs} "
                    "documento(s) selecionado(s) "
